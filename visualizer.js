class Visualizer{
    static drawNetwork(ctx,network){
        ...
        const levelHeight=height/network.lCount;
        for(let i=network.lCount-1;i>=0;i--){
            const levelTop=top+
                lerp(
                    height-levelHeight,
                    0,
    ...
    static drawLevel(ctx,level,left,top,width,height,outputLabels){
        ...
        const inputs = level.in;
        const outputs=level.out;
        const weights = level.w;
        const biases = level.b;
